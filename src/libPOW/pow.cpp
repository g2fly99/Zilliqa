...
ethash_mining_result_t POW::MineLightGpu(ethash_light_t& light,
                                      ethash_h256_t const& header_hash,
                                      ethash_h256_t& difficulty)
{
    uint64_t nonce = std::time(0);
    ...
    uint64_t result ;
    ethash_return_value_t mineResult ;
    status = EthashLightGpu(&result, light, header_hash, *difficulty, nonce);
    if (0 != status)
    {
        ETHASH_CRITICAL("failed Miner with GPU\n");
        goto FAILED;
    }

    mineResult = EthashLightCompute(light, header_hash, result);
    if (ethash_check_difficulty(&mineResult.result, &difficulty))
    {
        ethash_mining_result_t winning_result
            = {BlockhashToHexString(&mineResult.result),
